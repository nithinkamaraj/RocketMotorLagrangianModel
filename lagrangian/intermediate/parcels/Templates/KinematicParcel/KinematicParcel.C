...
////////////////////////////////////////////////////////////////////////////////
    // work transfer  from the particle to the carrier phase
    scalar dWTrans = 0.0;
////////////////////////////////////////////////////////////////////////////////
  ...
  //  Info << "Hi - KP" << endl;
     ...
     dWTrans = calcWorkTransfer(cloud, td, dt, Re, td.muc(), mass0, Su, dUTrans, Spu);
    ...
    if (cloud.solution().coupled())
    {
       ...
       cloud.WTrans()[this->cell()] +=np0*dWTrans;
...
//Info<<"Effective Mass of the system"<< massEff <<endl;
    /*
    // Proper splitting ...
   ...
   const vector deltaUcp = deltaU - deltaUncp;
	...
	//const vector k1  = (acp+ancp)-bcp*U_ ;

	//const  vector k2 = (acp+ancp)-bcp*(U_+0.5*dt*k1) ;

	//const vector k3 = (acp+ancp)-bcp*(U_+0.5*dt*k2) ;

	//const vector k4 = (acp+ancp)-bcp*(U_+dt*k3) ;

	//const vector deltaU = dt*(k1+2*k2+2*k3+k4)/6;

	//const vector deltaUcp = deltaU;
...
// scalar dKEdt  = 0.5*massEff/dt *( (Unew & Unew) - (this->U_ & this->U_)) ;

//Info << "dKEdt       "<< dKEdt<< endl;
  //Info << "momentum transfer (F)  "<< dUTrans/dt<<endl;

   //Info << "Drag Force          "<< -Fcp.Sp()*(td.Uc()-Unew)<< endl;
...
//  scalar dWTrans = Fcp.Sp()*(td.Uc()-Unew)&Unew;

//Info << "-------------------------------calculating new particle velocity! ----------------------" << Unew << endl;
...
template<class ParcelType>
template<class TrackCloudType>
Foam::scalar Foam::KinematicParcel<ParcelType>::calcWorkTransfer
(
    TrackCloudType& cloud,
    trackingData& td,
    const scalar dt,
...
// Info << "-------------------------------calculating Work transfer! ----------------------" << endl;
//
// Info<<"Fcp.Sp          "<< Fcp.Sp()<<endl;
// Info<<"Uc              "<< td.Uc()<<endl;
// Info<<"Uparticle       "<< this->U_<<endl;
// Info<<"du.u            "<< ((td.Uc() - this->U_) & this->U_) <<endl;

//scalar power = Fcp.Sp()*((td.Uc() - this->U_) & (td.Uc() - this->U_));



// scalar Vdt = this->mesh().cellVolumes()[this->cell()]*dt;

//Info<<"work source term        "<< power<<endl;
